import cv2
import numpy as np

# --- HSV ranges dictionary ---
COLOR_RANGES = {
    "blue":   ([np.array([75, 150, 0]),   np.array([130, 255, 255])]),
    "green":  ([np.array([35, 100, 100]), np.array([85, 255, 255])]),
    "yellow": ([np.array([20, 100, 100]), np.array([30, 255, 255])]),
    "red1":   ([np.array([0, 120, 70]),   np.array([10, 255, 255])]),
    "red2":   ([np.array([170, 120, 70]), np.array([180, 255, 255])]),
}

# --- Open webcam ---
vs = cv2.VideoCapture(0)
vs.set(cv2.CAP_PROP_FRAME_WIDTH, 320)
vs.set(cv2.CAP_PROP_FRAME_HEIGHT, 240)

width  = int(vs.get(cv2.CAP_PROP_FRAME_WIDTH))
height = int(vs.get(cv2.CAP_PROP_FRAME_HEIGHT))

frame_center_x = width // 2
frame_center_y = height // 2

# For smoothing
prev_cx, prev_cy = None, None
alpha = 0.4   # smoothing factor (0.0 = very smooth, 1.0 = no smoothing)

while True:
    ret, frame = vs.read()
    if not ret:
        break

    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)

    detected_color = None
    largest_area = 0
    best_box = None
    best_cnt = None

    # --- Check each color ---
    for color, bounds in COLOR_RANGES.items():
        if color in ("red1", "red2"):
            continue  # handle red separately

        lower, upper = bounds
        mask = cv2.inRange(hsv, lower, upper)
        contours, _ = cv2.findContours(mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

        for cnt in contours:
            area = cv2.contourArea(cnt)
            if area > 2000 and area > largest_area:
                largest_area = area
                x, y, w, h = cv2.boundingRect(cnt)
                best_box = (x, y, w, h)
                best_cnt = cnt
                detected_color = color

    # --- Special case: red (two ranges) ---
    lower1, upper1 = COLOR_RANGES["red1"]
    lower2, upper2 = COLOR_RANGES["red2"]
    mask1 = cv2.inRange(hsv, lower1, upper1)
    mask2 = cv2.inRange(hsv, lower2, upper2)
    mask_red = mask1 | mask2
    contours, _ = cv2.findContours(mask_red, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    for cnt in contours:
        area = cv2.contourArea(cnt)
        if area > 1000 and area > largest_area:
            largest_area = area
            x, y, w, h = cv2.boundingRect(cnt)
            best_box = (x, y, w, h)
            best_cnt = cnt
            detected_color = "red"

    # --- Draw result ---
    if detected_color and best_box and best_cnt is not None:
        x, y, w, h = best_box
        cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2)
        cv2.putText(frame, detected_color.upper(), (x, y-10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 0, 255), 2)

        # Moments for center
        M = cv2.moments(best_cnt)
        if M["m00"] != 0:
            cx = int(M["m10"] / M["m00"])
            cy = int(M["m01"] / M["m00"])

            # Smooth center with exponential moving average
            if prev_cx is None:
                prev_cx, prev_cy = cx, cy
            else:
                cx = int(alpha * cx + (1 - alpha) * prev_cx)
                cy = int(alpha * cy + (1 - alpha) * prev_cy)
                prev_cx, prev_cy = cx, cy

            # Draw object center
            cv2.circle(frame, (cx, cy), 5, (255, 0, 0), -1)
            cv2.putText(frame, f"({cx}, {cy})", (cx+10, cy+10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)

            # Draw frame center
            cv2.circle(frame, (frame_center_x, frame_center_y), 5, (0, 255, 255), -1)
            cv2.putText(frame, "Center", (frame_center_x+10, frame_center_y+10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)

            print("Detected:", detected_color, "at", (cx, cy))

    cv2.imshow("Frame", frame)

    key = cv2.waitKey(1)
    if key == 27:  # ESC to exit
        break

vs.release()
cv2.destroyAllWindows()
